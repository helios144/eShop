	    	...
	    	<div class="container">
	    		<div class="row">
	    			...
	    			<?php
							...
							$sql="SELECT name, description, kaina,nuotrauka,ID FROM prekes WHERE ID=".$_GET['ID'];
	    			...
	    			<div class="col-sm-6">
	    				<div class="product-image-large">
	    					<img id="preke_img" src="'.$row['nuotrauka'].'" alt="Item Name">
	    				</div>
	    			...
	    			<div class="col-sm-6 product-details">
		    			<div class="form-group">
		    				...
		    				echo '<h4>Category:</h4>
		    				<select onchange="this.form.submit()" id="selectas" class="form-control" name="cat">';
							$sqll = "SELECT kategorija,ID FROM kategoriju_sarasas";
							$resultt = $conn->query($sqll);
							if ($resultt->num_rows > 0) {
								// output data of each row
								while($roww = $resultt->fetch_assoc()) {
									$categories[]=$roww['kategorija'];
								}
							}
							...
							$sqll = 'SELECT kategorijos.Kategorija,kategoriju_sarasas.ID FROM kategorijos INNER JOIN kategoriju_sarasas ON kategorijos.Kategorija=kategoriju_sarasas.kategorija where preke_id='.$_GET['ID'];
							$resultt = $conn->query($sqll);
							if ($resultt->num_rows > 0) {
								// output data of each row
								while($roww = $resultt->fetch_assoc()) {
									$itemcategory=$roww['Kategorija'];
									$catt=$roww['ID'];

								}
							}
							echo "<script>document.getElementById('selectas').value = '".$itemcategory."';</script>";
							echo '<h4>Attribute:</h4>
		    				<select id="atributas" class="form-control" name="att">';
		    				$sql = "SELECT atributu_sarasas.ID,atributas,kategorijos_ID FROM atributu_sarasas
						INNER JOIN kategoriju_sarasas ON atributu_sarasas.kategorijos_ID=kategoriju_sarasas.ID WHERE kategorijos_ID=".$catt;
						$result = $conn->query($sql);
						if ($result->num_rows > 0) {		
						    // output data of each row
						    while($rowww = $result->fetch_assoc()) {
						    	echo "<option value='".$rowww['ID']."'>".$rowww['atributas']."</option>";
						    }
						}
						echo "</select>";
						$sql='SELECT atributas_id FROM atributai_kategorijai WHERE preke_id='.$_GET['ID'];
						$resultt = $conn->query($sql);
						$roow = $resultt->fetch_assoc();
						echo "<script>document.getElementById('atributas').value = '".$roow['atributas_id']."';</script>";
							//------------------------------------
	    				echo '</div>
	    				<div class="price">
							 Price: <input type="text" name="kaina" value="'.$row['kaina'].'"></input>&euro;
						</div>
						<h5>Product Description</h5>
	    				<p>
	    				<div class="form-group">
	    				<textarea type="text "name="description" class="form-control" rows="8">'.$row['name'].'</textarea>
	    				</div>
	    				</p>
	    				<button class="btn btn-large" type="submit" name="change" value="'.$_GET['ID'].'">Edit</button>
	    			</div>
